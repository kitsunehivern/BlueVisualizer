...
void CLLVisualizer::insertAtTheFront(int value) {
	...
	for (int i = 0; i < edges.size(); i++) {
		...
		// Edge
		action.draw(&edges, 1, i, &assets->highlightEdgeColor);
		action.draw(&edges, i + 1, edges.size() - 1, &assets->normalEdgeColor);
		action.draw(&randomCircularEdge, &assets->normalEdgeColor);
		...
		// Label
		if (i == 0) {
			action.drawChange(&labels, 1, &assets->labelColor, "head/pre", "0/head/pre");
		} else {
			action.draw(&labels, 1, &assets->labelColor, "head");
			action.drawChange(&labels, i + 1, &assets->labelColor, "pre", std::to_string(i) + "/pre");
		}
		...
		action.draw(&edges, 1, i, &assets->highlightEdgeColor);
		action.draw(&edges, i + 1, edges.size() - 1, &assets->normalEdgeColor);
		action.drawSlideIn(&edges, i + 1, &assets->highlightEdgeColor);
		action.draw(&randomCircularEdge, &assets->normalEdgeColor);
		...
		// Label
		if (i == 0) {
			action.drawChange(&labels, 1, &assets->labelColor, "0/head/pre", "head");
		} else {
			action.draw(&labels, 1, &assets->labelColor, "head");
			action.drawFadeOut(&labels, i + 1, &assets->labelColor, std::to_string(i) + "/pre");
		}

		action.drawFadeIn(&labels, i + 2, &assets->labelColor, "pre");
	...
	// Edge
	action.draw(&edges, 1, edges.size() - 1, &assets->highlightEdgeColor);
	action.draw(&randomCircularEdge, &assets->normalEdgeColor);
	...
	// Label
	if (nodes.size() == 2) {
		action.drawChange(&labels, 1, &assets->labelColor, "0/head/pre", "head/pre");
	} else {
		action.draw(&labels, 1, &assets->labelColor, "head");
		action.drawChange(&labels.back(), &assets->labelColor, std::to_string(labels.size() - 2) + "/pre", "pre");
	}
	...
	// Label
	if (nodes.size() == 2) {
		action.draw(&labels, 1, &assets->labelColor, "head/pre");
	} else {
		action.draw(&labels, 1, &assets->labelColor, "head");
	...
	// Label
	if (nodes.size() == 2) {
		action.drawChange(&labels, 1, &assets->labelColor, "head/pre", "pre");
	} else {
	...
	action.drawChange(&edges, 1, edges.size() - 1, &assets->highlightEdgeColor, &assets->normalEdgeColor);
...
void CLLVisualizer::insertAtTheBack(int value) {
	...
	// Edge
	action.draw(&edges, 0, edges.size() - 2, &assets->highlightEdgeColor);
	action.draw(&edges.back(), &assets->highlightEdgeColor);
	action.drawSlideIn(&circularEdge, &assets->insertedEdgeColor);
	...
	action.drawChange(&edges.back(), &assets->highlightEdgeColor, &assets->normalEdgeColor);
	action.drawChange(&circularEdge, &assets->insertedEdgeColor, &assets->normalEdgeColor);
...
void CLLVisualizer::eraseAtTheFront() {
	...
	for (int i = 0; i <= nodes.size(); i++) {
		...
		// Edge
		action.draw(&edges, 0, i - 2, &assets->highlightEdgeColor);
		if (i == 0) {
			action.draw(&deletedEdge, &assets->normalEdgeColor);
			action.draw(&edges, 0, edges.size() - 1, &assets->normalEdgeColor);
		} else {
			action.draw(&deletedEdge, &assets->highlightEdgeColor);
			action.draw(&edges, i - 1, edges.size() - 1, &assets->normalEdgeColor);
		}
		...
		// Label
		if (i == 0) {
			action.drawChange(&deletedLabel, &assets->labelColor, "head/pre", "0/head/pre");
		} else {
			action.draw(&deletedLabel, &assets->labelColor, "head");
			action.drawChange(&labels, i - 1, &assets->labelColor, "pre", std::to_string(i) + "/pre");
		}
		...
		if (i == 0) {
			action.draw(&deletedEdge, &assets->normalEdgeColor);
			action.drawSlideIn(&deletedEdge, &assets->highlightEdgeColor);
			action.draw(&edges, 0, edges.size() - 1, &assets->normalEdgeColor);
		} else {
			action.draw(&deletedEdge, &assets->highlightEdgeColor);
			action.draw(&edges, i - 1, edges.size() - 1, &assets->normalEdgeColor);
		...
		// Label
		if (i == 0) {
			action.drawChange(&deletedLabel, &assets->labelColor, "0/head/pre", "head");
		} else {
			action.draw(&deletedLabel, &assets->labelColor, "head");
			action.drawFadeOut(&labels, i - 1, &assets->labelColor, std::to_string(i) + "/pre");
		}

		action.drawFadeIn(&labels, i, &assets->labelColor, "pre");
	...
	// Label
	action.drawChange(&deletedLabel, &assets->labelColor, "head", "head/del");
	action.drawChange(&labels.back(), &assets->labelColor, std::to_string(labels.size()) + "/pre", "pre");
	...
	action.drawChange(&deletedLabel, &assets->labelColor, "head/del", "del");
	if (labels.size() == 1) {
		action.drawChange(&labels.back(), &assets->labelColor, "pre", "head/pre");
	} else {
		action.drawFadeIn(&labels.front(), &assets->labelColor, "head");
		action.draw(&labels.back(), &assets->labelColor, "pre");
	}
	...
	action.drawFadeOut(&deletedLabel, &assets->labelColor, "del");
	if (labels.size() == 1) {
		action.draw(&labels.back(), &assets->labelColor, "head/pre");
	} else {
		action.draw(&labels.front(), &assets->labelColor, "head");
		action.draw(&labels.back(), &assets->labelColor, "pre");
	}
	...
	// Label
	if (labels.size() == 1) {
		action.drawChange(&labels.back(), &assets->labelColor, "head/pre", "head");
	} else {
		action.draw(&labels.front(), &assets->labelColor, "head");
		action.drawFadeOut(&labels.back(), &assets->labelColor, "pre");
	}
...
void CLLVisualizer::eraseAtTheBack() {
	...
	randomCircularEdge.left = &nodes.front();
	randomCircularEdge.right = &deletedNode;
...
void CLLVisualizer::eraseAtTheMiddle(int index) {
